...
export const transferTransaction = async (tokenId: number) => {
  ...
  // get EOA address from wallet provider
    ...
    const nftInterface = new ethers.utils.Interface([
      "function transferFromOrgin(address, address, uint256)",
    ]);
  
    // Here we are minting NFT to smart account address itself
    const myaddress = await signer.getAddress();
    const data = nftInterface.encodeFunctionData("transferFromOrgin", [myaddress.toLowerCase(), "0x7DFcAaC23908ed3fC1730560a86AE1d40784ebe9".toLowerCase(), tokenId]);
